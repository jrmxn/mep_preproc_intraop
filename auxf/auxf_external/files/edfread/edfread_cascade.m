function [hdr, record] = edfread_cascade(fname, varargin)
% modification of edfread.m to deal with cascade iomax data more elegantly.
% mcintosh 2020-03-02
...
startdate  = fread(fid,8,'*char')';% (dd.mm.yy)
% hdr.startdate  = datestr(datenum(fread(fid,8,'*char')','dd.mm.yy'), 29); %'yyyy-mm-dd' (ISO 8601)
starttime  = fread(fid,8,'*char')';% (hh.mm.ss)
dt_start = datetime(...
    sprintf('%s_%s', startdate, starttime), ...
    'InputFormat', 'dd.MM.yy_HH.mm.ss');
...
% 2020-03-02 mcintosh
assert(strcmpi(strtrim(char(reserved)'), 'EDF+D'), 'Is this a CASCADE IOMAX Discontinuous EDF+D file?');
...
if hdr.records == -1
    ...
    [hdr, tmpdata] = edfreadUntilDone(fname, varargin);
    return
end
...
% 2020-03-02 mcintosh
case_annot = strcmpi(hdr.label, 'EDFAnnotations');
targetSignals = targetSignals(not(case_annot));
assert(sum(case_annot)<=1, 'Only one annot allowed!');
...
if nargout > 1 || assignToVariables
    ...
    for recnum = 1:hdr.records
        for ii = 1:hdr.ns
            ...
            if case_annot(ii)  % 2020-03-02 mcintosh - added to read annotations
                str_type = 'uint8';
                annot_temp = fread(fid, hdr.samples(ii) * 2, str_type);
                % 2020-03-02 mcintosh
                % not sure if cascade error, or just they use this control
                % char for something else, but to make the string format
                % nicer...
                annot_temp(annot_temp == 20) = 32; % convert dec 20 (HEX 14) to dec 32 (HEX 20)
                tmpdata(recnum).annotation = native2unicode(annot_temp)';
            elseif ismember(ii,targetSignals)
                % Use a cell array for DATA because number of samples may vary
                % from sample to sample
                data_temp = fread(fid, hdr.samples(ii), 'int16');
                data_temp = data_temp  * scalefac(ii) + dc(ii);
                tmpdata(recnum).data{ii} = data_temp;
            else
    ...
    % 2020-03-02 mcintosh - don't merge records, just do this:
    record = cell(1, hdr.records);
    for recnum = 1:hdr.records
        record{recnum} = cell2mat(tmpdata(recnum).data)';
    end
    
    hdr.annotation = cell(1, hdr.records);
    hdr.delay = nan(1, hdr.records);
    hdr.datetime = NaT(1, hdr.records);
%     dt_start = hdr.datetime;
    for recnum = 1:hdr.records
        hdr.annotation{recnum} = tmpdata(recnum).annotation;
        str_delay = regexp(hdr.annotation{recnum}, '\+(\d+\.?\d*)\s+', 'tokens');
        str_delay = str_delay{1}{1};
        hdr.delay(recnum) = str2double(str_delay);
        hdr.datetime(recnum) = dt_start + hdr.delay(recnum) * seconds;
    end
    ...
    if assignToVariables
        ...
        tmpdata = [];% No need to output record data as a matrix?
    end
end
fclose(fid);
end