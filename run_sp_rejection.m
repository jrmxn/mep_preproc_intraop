...
config_struct.participant = string(config_struct.participant);

%%
vec_participant = string(config_struct.participant);
    ...
    for ix_particpant = 1:length(vec_participant)
        ...
        if any(strcmpi(config_struct.reject_mode, reject_mode))
            sp.rejection(participant, 'reject_mode', reject_mode, ...
                'ephys_mode', vec_ephys_mode(1), 'fs_lowpass', fs_lowpass, ...
